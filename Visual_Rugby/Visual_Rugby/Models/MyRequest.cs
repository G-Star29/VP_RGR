...
{
    public class MyRequest: DataTable, IReactiveObject
    {
        ...
        private void GenerateQuery()
        {
            ...
            for(int i=1; i<Items.Count; i++)
            {
                ...
                if(Joins[i-1].secondTable as MyRequest != null)
                {
                    (Joins[i - 1].secondTable as MyRequest).GenerateQuery();
                    tbn = "(" + (Joins[i - 1].secondTable as MyRequest).QueryString + ") as "+tbn;
                }
        ...
        public ObservableCollection<MyRequestItem> Items
        {
            get;
            set;
        }
        public MyRequest(string name) :base()
        {
            TableName = name;
            Items = new ObservableCollection<MyRequestItem>();
